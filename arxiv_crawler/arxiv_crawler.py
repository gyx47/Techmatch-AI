...
class ArxivScraper(object):
    def __init__(
        ...
        self.search_from_date = datetime.strptime(date_from, "%Y-%m-%d")
        self.search_until_date = datetime.strptime(date_until, "%Y-%m-%d")
        ...
        self.should_stop = None  # 停止标志检查函数
    ...
    def get_url(self, start):
        ...
        date_from = self.search_from_date.strftime("%Y-%m-%d")
        # date_until = (self.search_from_date + timedelta(days=5)).strftime("%Y-%m-%d")
        date_until = self.search_until_date.strftime("%Y-%m-%d")
        return (
    ...
    async def request(self, session, start):
        """
        异步请求网页，重试至多3次，使用指数退避
        session: aiohttp.ClientSession 实例，由外部创建并复用
        """
        error = 0
        url = self.get_url(start)
        timeout = aiohttp.ClientTimeout(total=60, connect=15)  # 增加超时时间：总超时60秒，连接超时15秒
        while error < 3:
            try:
                ...
                if error < 3:
                    ...
                    wait_time = 2 ** error
                    self.console.log(f"[bold yellow]Request {start} failed, retrying {error}/3... (waiting {wait_time}s)")
                    await asyncio.sleep(wait_time)
                else:
                    self.console.log(f"[bold red]Request {start} failed after 3 retries")
    ...
    async def fetch_all(self):
        ...
        (aio)获取所有文章
        """
        # 创建共享的 ClientSession，在整个爬取过程中复用
        ...
        async with aiohttp.ClientSession(trust_env=True, timeout=timeout) as session:
            ...
            content = await self.request(session, 0)
            self.papers.extend(self.parse_search_html(content))
            
            # 检查是否有结果
            if self.total == 0:
                self.console.log(f"[bold yellow]未找到匹配的论文")
                self.console.log(f"[yellow]可能的原因：")
                self.console.log(f"[yellow]  1. 日期范围内没有符合条件的论文")
                self.console.log(f"[yellow]  2. 关键词或领域过滤太严格")
                self.console.log(f"[yellow]  3. arXiv 服务器暂时无法访问")
                return  # 提前返回，不执行后续处理

            # 如果只有第一页的结果，不需要继续获取
            if self.total <= self.step:
                self.console.log(f"[bold green]找到 {self.total} 篇论文，无需继续获取")
            else:
                # 获取剩余的内容
                with Progress(
                    SpinnerColumn(),
                    *Progress.get_default_columns(),
                    ...
                    sem = asyncio.Semaphore(5)  # 降低并发数到5，避免被arXiv限流
                    async def wrapper(start):  # wrapper用于显示进度
                        ...
                        async with sem:
                            ...
                            if self.should_stop and self.should_stop():
                                return []
                            # 在请求之间添加小延迟，避免请求过快
                            await asyncio.sleep(0.2)  # 每个请求之间延迟200ms
                            content = await self.request(session, start)
                            papers = self.parse_search_html(content)
                            p.update(task, advance=self.step)
                            return papers

                    # 分批处理任务，避免一次性创建太多任务导致内存和网络压力
                    batch_size = 20  # 每批处理20个任务
                    all_papers = []
                    starts = list(range(self.step, self.total, self.step))
                    
                    for i in range(0, len(starts), batch_size):
                        # 检查是否应该停止
                        if self.should_stop and self.should_stop():
                            self.console.log(f"[bold yellow]收到停止请求，已处理 {len(all_papers) * self.step} 篇论文")
                            break
                        
                        batch_starts = starts[i:i + batch_size]
                        batch_tasks = [wrapper(start) for start in batch_starts]
                        batch_papers = await asyncio.gather(*batch_tasks)
                        all_papers.extend(chain(*batch_papers))
                        
                        # 批次之间稍作休息，避免请求过快
                        if i + batch_size < len(starts):
                            await asyncio.sleep(1)  # 每批之间休息1秒
                    
                    self.papers.extend(all_papers)

        # 只有在有论文的情况下才处理
        if len(self.papers) > 0:
            self.console.log(f"[bold green]Fetching completed. 共找到 {len(self.papers)} 篇论文")
            # if self.trans_to:
            #     await self.translate()
            self.process_papers()
        else:
            self.console.log(f"[bold yellow]未找到任何论文")

    async def fetch_update_async(self):
        """
        异步版本的更新文章方法
        ...
        timeout = aiohttp.ClientTimeout(total=30, connect=10)
        async with aiohttp.ClientSession(trust_env=True, timeout=timeout) as session:
            continue_update = await self.update_async(session, 0)
            for start in range(self.step, self.total, self.step):
                if not continue_update:
                    break
                continue_update = await self.update_async(session, start)
    ...
    def fetch_update(self):
        ...
        同步包装器，调用异步版本
        """
        asyncio.run(self.fetch_update_async())
    ...
    async def update_async(self, session, start) -> bool:
        """
        异步版本的update方法，复用session
        """
        content = await self.request(session, start)
        self.papers.extend(self.parse_search_html(content))
    ...
    def update(self, start) -> bool:
        """
        同步版本的update方法（保持向后兼容）
        注意：这个方法每次调用都会创建新的事件循环和session，效率较低
        建议使用 fetch_update_async 或直接使用异步方法
        """
        timeout = aiohttp.ClientTimeout(total=30, connect=10)
        async def _update():
            async with aiohttp.ClientSession(trust_env=True, timeout=timeout) as session:
...
def run_crawler(keywords: list, date_from: str, date_until: str):

            print(f"开始爬取，关键词: {keywords}, 日期范围: {date_from} to {date_until}")
...
if __name__ == "__main__":
    ...
    seven_days_ago = today - timedelta(days=40)
    scraper = ArxivScraper(
